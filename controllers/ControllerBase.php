...
abstract class ControllerBase {
    ...
    public function callMethod($method, $caller='Action') {
        ...
        try {
            ...
            global $smarty;
            $err = $caller . "Error";
            //$e->setMessage($smarty->fetch('error.tpl'));
            debug($e);
            $e2 = new Exception($smarty->fetch('error.tpl'), $e->getCode());
    ...
    private function JsonError($e) {
        ...
        $response['message'] = $e->getMessage(); //$smarty->fetch('error.tpl');
    ...
    public function XmlRun($actionName) {
        ...
        try {
            ...
            if ($aux instanceof ORMBase)
                $aux = $aux->toXML();
            if ($aux instanceof ORMCollection)
                $aux = $aux->toXML();
            if (!( $aux instanceof DOMDocument ))
                $aux = $this->toXML($aux);
