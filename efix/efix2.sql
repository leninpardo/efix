...
INSERT INTO `ambientes` VALUES ('5', 'pisop prueba', null, '6', null, '1');
INSERT INTO `ambientes` VALUES ('6', 'piso2', 'aula 01', '6', '5', '1');
INSERT INTO `ambientes` VALUES ('7', 'pisw', null, '6', null, '1');
INSERT INTO `ambientes` VALUES ('8', 'pisox', null, '6', null, '1');
INSERT INTO `ambientes` VALUES ('9', '', 'aula 02', '6', '5', '1');
INSERT INTO `ambientes` VALUES ('10', null, 'aula03', '6', '5', '1');
...
CREATE  VIEW `vista_ambiente` AS select `ambientes`.`idambientes` AS `idambientes`,`ambientes`.`piso` AS `piso`,`facultad`.`nombre` AS `ambiente` from (`ambientes` join `facultad` on((`ambientes`.`facultad` = `facultad`.`idfacultad`))) where ((`ambientes`.`estado` <> 0) and isnull(`ambientes`.`codigo_aula`)) ;

-- ----------------------------
-- View structure for vista_aula
-- ----------------------------
DROP VIEW IF EXISTS `vista_aula`;
CREATE  VIEW `vista_aula` AS select `ambientes`.`idambientes` AS `nro`,`ambientes`.`aulas` AS `aulas`,`am`.`piso` AS `piso`,`facultad`.`nombre` AS `facultad` from ((`ambientes` join `facultad` on((`facultad`.`idfacultad` = `ambientes`.`facultad`))) join `ambientes` `am` on((`am`.`idambientes` = `ambientes`.`codigo_aula`))) where (`ambientes`.`estado` <> 0) ;

-- ----------------------------
-- View structure for vista_averia
-- ----------------------------
DROP VIEW IF EXISTS `vista_averia`;
CREATE  VIEW `vista_averia` AS select `averias`.`idaverias` AS `codigo_averia`,`averias`.`fecha` AS `fecha`,`averias`.`descripcion` AS `descripcion`,(case `averias`.`estado` when 1 then 'software' when 2 then 'hardware' when 3 then 'red' end) AS `tipo`,`ambientes`.`piso` AS `ambiente`,`facultad`.`nombre` AS `facultad`,`averias`.`estado` AS `estado` from (((`ambientes` join `ambientes` `am` on((`am`.`codigo_aula` = `ambientes`.`codigo_aula`))) join `averias` on((`averias`.`ambiente` = `am`.`idambientes`))) join `facultad` on((`facultad`.`idfacultad` = `am`.`facultad`))) ;

-- ----------------------------
-- View structure for vista_facultad
-- ----------------------------
DROP VIEW IF EXISTS `vista_facultad`;
CREATE  VIEW `vista_facultad` AS select `facultad`.`idfacultad` AS `nro`,`facultad`.`nombre` AS `nombre`,`facultad`.`longitud` AS `longitud`,`facultad`.`latitud` AS `latitud`,`facultad`.`altura` AS `altura` from `facultad` where (`facultad`.`estado` <> 0) ;

-- ----------------------------
-- View structure for vista_modulo
-- ----------------------------
DROP VIEW IF EXISTS `vista_modulo`;
CREATE  VIEW `vista_modulo` AS select `modulo`.`idmodulo` AS `idmodulo`,`modulo`.`descripcion` AS `descripcion` from `modulo` where isnull(`modulo`.`submodulo`) ;

-- ----------------------------
-- View structure for vista_modulos
-- ----------------------------
DROP VIEW IF EXISTS `vista_modulos`;
CREATE  VIEW `vista_modulos` AS select `m`.`idmodulo` AS `idmodulo`,`mm`.`descripcion` AS `padre`,`m`.`descripcion` AS `descripcion`,`m`.`url` AS `url`,`m`.`orden` AS `orden`,(case `m`.`estado` when '1' then 'Activo' else 'Inactivo' end) AS `estado` from (`modulo` `m` left join `modulo` `mm` on((`mm`.`idmodulo` = `m`.`submodulo`))) ;

-- ----------------------------
-- View structure for vista_perfil
-- ----------------------------
DROP VIEW IF EXISTS `vista_perfil`;
CREATE  VIEW `vista_perfil` AS select `perfil`.`idperfil` AS `nro`,`perfil`.`descripcion` AS `perfil` from `perfil` where (`perfil`.`estado` = 1) ;

-- ----------------------------
-- View structure for v_usuario
-- ----------------------------
DROP VIEW IF EXISTS `v_usuario`;
CREATE  VIEW `v_usuario` AS select `u`.`idusuario` AS `item`,`p`.`descripcion` AS `Perfil`,`u`.`nombres` AS `Usuario`,`u`.`profesion` AS `profesion` from (`usuario` `u` join `perfil` `p`) where (`u`.`perfil` = `p`.`idperfil`) ;
...
DELIMITER ;;
CREATE  PROCEDURE `usp_modulo`(op  int,id int,submodulo int,descripcion text,url text,orden int,estado int)
BEGIN
declare max_id int;
if(op=1)then 
...
DELIMITER ;;
CREATE  PROCEDURE `usp_perfil`(op  int,descrip text,id int)
BEGIN
declare max_id int;
if(op=1)then 
...
DELIMITER ;;
CREATE  PROCEDURE `usp_permiso`(_perfil int,_modulo int,_modificar int,_eliminar int,_insertar int,_acceder int,_permiso int)
BEGIN
...
DELIMITER ;;
CREATE  PROCEDURE `usp_usuario`(op int,_perfil int,_nombres text,_profesion text,_domicilio text,_telefono text,_dni text,_usuario text,_clave text,_fecha_n date,_foto text, id int)
BEGIN
	#Routine body goes here...
DECLARE max_id int; 
if(op=1)then
