   ...
   function asign($titulo,$controller,$cabecera,$rows,$p,$pag,$query,$view,$imprimir=false)
	 ...
	 function _body()
	{
      ...
      $html.="<tr>";
	   ...
	   $html.=" <td width='50px' style='font-size:13px;font-family:arial;'>".'Atender'."</td>";
	  ...
	  // if($_SESSION['delete']==1){ $html.=" <td width='40px' style='font-size:13px;font-family:arial;'>".'eliminar'."</td>";}
       ...
       foreach($this->data['rows'] as $val)
       {
	      ...
	      if($val[6]==1){$bgcolor='#EE6464';}else{$bgcolor='#F5F5F5';}
          $c++;
         ...
         onMouseOver=\"this.style.backgroundColor='#CCC';this.style.cursor='pointer';\" onMouseOut=\"this.style.backgroundColor='".$bgcolor."'\"o\"];\">";
		   foreach($this->data['cabecera'] as $key=>$t)
		   {
			...
			if($key==2){$html.="<td>".($val[$key])."</td>"; }	
			if($key==3){$html.="<td>".($val[$key])."</td>";}	
			if($key==4){$html.="<td align='center'>".($val[$key])."</td>";}	
			...
			if($key==6){
                            ...
                            if($val[$key]==1){ $msg = '<span style="color:red">'.$estado.'</span>';}
							else{$msg = '<span style="color:blue">'.$estado.'</span>';}
							$html.="<td>".$msg."</td>";
					}
			//if($key==7){$html.="<td>".($val[$key])."</td>";}		
		   }
			
		 if($val[6]==1){$html.=" <td align='center'><a href=\"javascript:_atender ('index.php?controller=". $this->data['controller']."&action=atender&id=".$val[0]."');\" title='Atender'><i class='icon-plus'></i></td>";}
		 else{$html.=" <td align='center'><a href=\"javascript:_msgatender('index.php?controller=". $this->data['controller']."&action=show&id=".$val[0]."');\" title='Ver'> <i class='icon-check'></i> </a></td>";}
