...
$.widget( "di.comboEdit", {
      ...
      _create: function() {
            ...
            this.options.ventana = $(document.createElement('div')).attr('id', 'frmVtn_' + this.options.num_ident).attr('title', this.options.titulo).html("<center><img src='/images/cargando.gif' alt='Cargando datos'/><br/>Cargando...</center>").append(this.options.formulario).dialog({
                    autoOpen: false,
          ...
          var este = this;
            ...
            if (this.options.cargado == false){
                this.options.ventana.empty().load(this.options.url_form,{},function(){
                    ...
                    if (este.options.div_form != null)
                        $('#' + este.options.div_form).attr('id', este.options.div_form + '_' + este.options.num_ident);
